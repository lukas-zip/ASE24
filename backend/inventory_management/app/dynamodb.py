            ...
            GlobalSecondaryIndexUpdates=[
                {
                    "Create": {
                        ...
                        "KeySchema": [
                            {"AttributeName": "product_category", "KeyType": "HASH"}
                        ],
                ...
                },
                {
                    "Create": {
                        ...
                        "KeySchema": [
                            {
                                "AttributeName": "product_search_attributes",
                                "KeyType": "HASH",
                            }
                        ],
                        "Projection": {"ProjectionType": "ALL"},
    ...
    try:
        ...
        for item in response.get("Items", []):
            ...
            product_info = {
                ...
                "product_current_stock": item.get("product_current_stock", {}).get(
                    "N", ""
                ),
                "product_should_stock": item.get("product_should_stock", {}).get(
                    "N", ""
                ),
                "product_price": item.get("product_price", {}).get("N", ""),
                "product_price_reduction": item.get("product_price_reduction", {}).get(
                    "N", ""
                ),
                "product_sale": item.get("product_sale", {}).get("BOOL", False),
                "product_category": item.get("product_category", {}).get("SS", ""),
                "product_search_attributes": item.get(
        ...
        while "LastEvaluatedKey" in response:
            ...
            for item in response.get("Items", []):
                ...
                product_info = {
                    ...
                    "product_name": item.get("product_name", {}).get("S", ""),
                    "product_picture": item.get("product_picture", {}).get("SS", ""),
                    "product_description": item.get("product_description", {}).get(
                        "S", ""
                    ),
                    "product_current_stock": item.get("product_current_stock", {}).get(
                        "N", ""
                    ...
                    "product_price": item.get("product_price", {}).get("N", ""),
                    "product_price_reduction": item.get(
                    ...
                    "product_reviews": item.get("product_reviews", {}).get("SS", ""),
                    "product_bom": item.get("product_bom", {}).get("SS", ""),
                    "product_assemblies": item.get("product_assemblies", {}).get(
                        "S", ""
                    ),
                }
                products[product_id] = product_info
...
def product_check(product_id):
    try:
        response = db_inventory_management.get_item(
            TableName="Products", Key={"product_id": {"S": product_id}}
        )
...
def add_product(
    ...
    try:
        ...
        if product_uuid == None:  # not for testing
        ...
        for existing_product_id, existing_product_info in existing_products.items():
            if existing_product_info["product_name"] == product_name:
                print(
                    f"A product with the same name already exists for {product_owner}."
                )
                return
        ...
        response = db_inventory_management.put_item(
            ...
            Item={
                ...
                "product_assemblies": {
                    "S": str(product_assemblies)
                },  # Baugruppe oder nicht
            },
        )
...
def delete_product(product_id):
    ...
    try:
        response = db_inventory_management.delete_item(
            TableName=table_name, Key={"product_id": {"S": product_id}}
        )
...
def perform_sell(product_id, product_owner, ordered_items):
    try:
        ...
        response = update_product(
            product_id, product_owner, {"product_current_stock": updated_stock}
        )
        if not response:
...
def search_products_by_category(category_term):
    try:
        response = db_inventory_management.scan(TableName="Products")
        ...
        for item in items:
            product_id = item.get("product_id", {}).get("S")
            categories = [
                cate.lower() for cate in item.get("product_category", {}).get("SS", [])
            ]
...
def search_products_by_attributes(attributes_term):
    try:
        ...
        for item in items:
            ...
            attributes = [
                attr.lower()
                for attr in item.get("product_search_attributes", {}).get("SS", [])
            ]
