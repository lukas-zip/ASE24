...
def test():
    ...
    return (
        jsonify(
            {"value": "Test was successful for the inventorymanagement", "status": True}
        ),
        200,
    )
...
def get_products_by_owner():
    ...
    products = dynamodb.get_products_by_product_owner(
        "1324a686-c8b1-4c84-bbd6-17325209d78c6"
    )

    # Format the products into a list of dictionaries
    final_products = [
        {"product_id": product_id, **product_info}
...
def insert_product():
    ...
    if None in (
        ...
        return (
            jsonify(
                {
                    "error": "All values need to at least contain one value.",
                    "status": False,
                }
            ),
            400,
    ...
    # Validate data types
    if (
        type(product_owner) != str
        or type(product_name) != str
        or type(product_description) != str
        or type(product_current_stock) != int
        or type(product_should_stock) != int
        or type(product_price) != float
        or type(product_picture) != list
        or type(product_price_reduction) != float
        or type(product_sale) != bool
        or type(product_category) != list
        or type(product_search_attributes) != list
        or type(product_reviews) != list
        or type(product_bom) != list
        or type(product_assemblies) != str
    ):
        return (
            jsonify(
                {
                    "error": "All values should need to be in expected dataformat.",
                    "status": False,
                }
            ),
            400,
    ...
    if (
        ...
        or product_should_stock < 0
        or product_price < 0
        or product_price_reduction < 0
    ):
        return (
            jsonify(
                {
                    "error": "The numeric values are not allowed to be negative.",
                    "status": False,
                }
            ),
            400,
...
def upload_picture():
    ...
    if object_type not in allowed_mime_types:
        return (
            jsonify({"error": "The dataformat cannot be accepted.", "status": False}),
            400,
        )
    ...
    try:
        ...
        if s3response:
            ...
            return (
                jsonify(
                    {
                        "value": "The image could unfortunately not be inserted",
                        "status": False,
                    }
                ),
                500,
    ...
    try:
        if dynamodb.product_check(product_id):
            ...
            for picture_path in product_data.get("product_picture", []):
                s3_object_key = picture_path.split("/")[
                    -1
                ]  # Extract object key from the picture path
                deletion_response = s3.delete_object(s3_object_key)
                if deletion_response:
                    print(
                        f"Product picture '{s3_object_key}' deleted successfully from S3."
                    )
                else:
            ...
            if response:
                return (
                    jsonify(
                        {
                            "value": "The item got deleted from the database.",
                            "status": True,
                        }
                    ),
                    200,
    ...
    try:
        ...
        if "product_owner" not in data:
            return (
                jsonify(
                    {
                        "error": "product_owner is required in the request body",
                        "status": False,
                    }
                ),
                400,
        ...
        if "amount" not in data:
            return (
                jsonify(
                    {"error": "amount is required in the request body", "status": False}
                ),
                400,
            )

        response = dynamodb.perform_sell(
            product_id, data["product_owner"], data["amount"]
        )
        if response:
            return (
                jsonify({"value": "successfully performed sell.", "status": True}),
                200,
            )
        else:
...
def get_products_to_sell_catalog(product_owner):
    ...
    try:
        ...
        final_products = [
            {"product_id": product_id, **product_info}
            for product_id, product_info in products.items()
            if product_info.get("product_assemblies") == "Final"
        ]
        if final_products:
            return jsonify({"value": final_products, "status": True}), 200
        else:
...
def get_products_to_build_catalog(product_owner):
    ...
    try:
        ...
        final_products = [
            {"product_id": product_id, **product_info}
            for product_id, product_info in products.items()
            if product_info.get("product_assemblies") == "Secondary"
        ]
        if final_products:
            return jsonify({"value": final_products, "status": True}), 200
        else:
...
def update_product_route(product_id):
    ...
    try:
        ...
        success = dynamodb.update_product(
            product_id, updated_data["product_owner"], updated_data
        )

        if success:
            return (
                jsonify({"value": "Product updated successfully", "status": True}),
                200,
            )
        else:
...
def search():
    ...
    if not term:
        return (
            jsonify({"error": "Search term parameter is required", "status": False}),
            400,
        )
...
def get_category():
    ...
    if not category:
        return (
            jsonify(
                {"error": "Search category parameter is required", "status": False}
            ),
            400,
        )
    ...
    formatted_results = [
        product_info for _, product_info in products_by_category.items()
    ]
...
def set_production(product_id):
    ...
    if not isinstance(data["amount"], int) or data["amount"] < 1:
        return (
            jsonify({"error": "The product seems not to exist", "status": False}),
            400,
        )
    ...
    try:
        ...
        if response:
            ...
            # update
            db_insertion = dynamodb.update_product(
                product_id, item["product_owner"], update_data
            )
            if db_insertion:
                return (
                    jsonify(
                        {
                            "value": "The production products have been inserted.",
                            "status": True,
                        }
                    ),
                    200,
                ...
                return (
                    jsonify(
                        {
                            "error": "The production products, could not be inserted.",
                            "status": False,
                        }
                    ),
                    400,
...
@route_blueprint.route(
    "/product/production/recommendations/<product_owner>", methods=["GET"]
)
def get_production_recommendations(product_owner):
    ...
    try:
        ...
        production_products = [
            {"product_id": product_id, **product_info}
            for product_id, product_info in products.items()
            if int(product_info.get("product_current_stock"))
            <= int(product_info.get("product_should_stock"))
        ]
...
# get all items in db,used for testing
@route_blueprint.route("/product", methods=["GET"])
def get_all_products_req():
    response = dynamodb.get_all_products()
