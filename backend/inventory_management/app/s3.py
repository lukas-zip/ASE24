...
db_s3_product_pictures = boto3.client(
    "s3",
...
def create_bucket():
    ...
    try:
        db_s3_product_pictures.create_bucket(Bucket=bucket_name)
...
def upload_fileobj(image_file, bucket_name, object_key):
    try:
        db_s3_product_pictures.upload_fileobj(image_file, bucket_name, object_key)
        return True
    except ClientError as e:
...
def bucket_exists(bucket_name):
    try:
        db_s3_product_pictures.head_bucket(Bucket=bucket_name)
        return True
    except ClientError as e:
...
def retrieve_image(object_key):
    ...
    try:
        ...
        response = db_s3_product_pictures.get_object(Bucket=bucket_name, Key=object_key)
